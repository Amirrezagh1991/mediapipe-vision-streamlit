...
import numpy as np
from models import FaceDetector, FaceLandmarkDetector, FaceStylization, SelfieSegmentation
import configs
...
def _helper(model_type):
    ...
    elif model_type == 'Face Stylization':
        detector = FaceStylization()
    return detector
...
def main():
    # Setting page layout
    st.set_page_config(
        page_title="Different Computer Vision Tasks with MediaPipe",
        page_icon="👁️",
        layout="wide",
        initial_sidebar_state="expanded"
    )

    st.title("Different Computer Vision Tasks with MediaPipe") #Main page heading
    st.sidebar.header("List of Available Tasks") # Sidebar

    # Model Options
    model_type = st.sidebar.radio(
        "Select Task", ['Face Detection', 'Face Landmarks', 'Selfie Segmentation', 'Face Stylization'])
    ...
    default_predicted_imgs = {'Face Detection': configs.DEFAULT_DETECT_IMAGE,
                              ...
                              'Face Stylization': configs.DEFAULT_STYLE_IMAGE,
                              }
